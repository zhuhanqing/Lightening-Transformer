...
def draw_bar_plot(data, ax, barwidth, barcolor, label=None, linestyle=None, tick_label=None):
    bp = ax.bar(
        data["x"],
        data["y"],
        align="center",
...
def draw_step_plot(data, ax, linewidth, linecolor, label=None, marker=None, markersize=1, linestyle=None, alpha=1
):
    bp = ax.step(
        data["x"],
        data["y"],
...
def draw_errorbar_plot(data, ax, linewidth, linecolor, label=None, linestyle=None,alpha=1):
    bp = ax.errorbar(
        data["x"],
        data["y"],
        yerr=data["yerror"],
...
def draw_pie_plot(data, ax, colors=None, explode=None, fontsize=10):
    if colors is None:
...
def draw_surface3d_plot(data, fig, ax, fontsize=10, zrange=[0, 1, 0.1], zformat="%.1f"):
    ...
    x, y, z = data["x"], data["y"], data["z"]
    X, Y = np.meshgrid(x, y)
    surf = ax.plot_surface(X, Y, z, cmap=plt.cm.RdYlGn, linewidth=0, antialiased=False)
...
def svg2pdf(src: str, dst: str) -> None:
    assert src.endswith(".svg"), print(f"[E] Source file must be SVG, but got {src}")
    assert dst.endswith(".pdf"), print(f"[E] Target file must be PDF, but got {dst}")
    drawing = svg2rlg(src)
...
def batch_plot(
    ...
    assert type in {
        ...
        "none",
        None,
    }, f"[E] Only support box, bar, line, scatter, errorbar, pie, mesh2d, surface3d chart, and none/None"
    if ieee:
    ...
    if type == "box":
        ...
        data = raw_data
        if smoothness > 1e-2:
            x, y = smooth_line(raw_data["x"], raw_data["y"], smoothness=smoothness)
            data["x"] = x
            data["y"] = y
        if gridwidth > 0:
            ax.grid(True, linewidth=gridwidth)
        ...
        if gridwidth > 0:
            ...
            ax.grid(False, linewidth=0)
        ax.set_axisbelow(True)
        bp = draw_errorbar_plot(data, ax, linewidth, trace_color, label=trace_label, alpha=alpha, linestyle=linestyle)
    elif type == "step":
        data = raw_data
        if gridwidth > 0:
            ax.grid(True, linewidth=gridwidth)
        ...
        data = raw_data
        bp = draw_pie_plot(data, ax, colors=pie_colors, explode=pie_explode, fontsize=fontsize)
    elif type == "mesh2d":
        data = raw_data
